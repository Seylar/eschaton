...
| 6. Zéro terminal | **OK pour update/rollback et surfaces DMS ; seule réserve : matériel audio absent de la VM** | Les parcours update/rollback ont été faits à la souris. Réseau et affichage sont exposés dans le Control Center. La VM est lancée avec `-audio none` : elle ne peut pas prouver le réglage d'un périphérique audio réel, seulement la présence de PipeWire/WirePlumber et de l'interface. **Réserve de rendu levée le 2026-08-28 (vm-dev §14.5)** : `eschaton-desktop-config` 0.1.0-9 détecte le décalage entre `settings.json` et le `barConfigs` en mémoire de DMS 1.5.3, puis demande une unique recomposition non bloquante (concrètement : un restart unique et détecté de `dms.service` — contournement d'un défaut amont : DMS 1.5.3 ne recharge pas `settings.json` à chaud et son IPC `settings set` refuse les tableaux ; **à remonter à DMS** : rechargement à chaud de `barConfigs` ou IPC acceptant les tableaux — issue à déposer, le contournement s'auto-neutralise le jour où l'amont l'apprend). Deux boots consécutifs depuis le paquet Pages montrent les pastilles update et rollback, le second sans nouvelle recomposition. |
