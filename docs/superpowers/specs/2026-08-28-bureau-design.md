...
| 6. Zéro terminal | **OK pour update/rollback et surfaces DMS ; réserve matérielle audio ; réserve nouvelle sur le rendu des pastilles** | Les parcours update/rollback ont été faits à la souris. Réseau et affichage sont exposés dans le Control Center. La VM est lancée avec `-audio none` : elle ne peut pas prouver le réglage d'un périphérique audio réel, seulement la présence de PipeWire/WirePlumber et de l'interface. **Réserve levée à découvrir avant le tag (2026-08-28, vm-dev §14.4)** : après un démarrage frais, les deux pastilles Eschaton sont bien *activées* (IPC `loaded`, `plugin_settings.json` `true,true`, `rightWidgets` complet) mais **ne sont pas rendues dans la barre** ; un `systemctl --user restart dms.service` les fait apparaître. Les assertions de la Task 7 ne portaient que sur les fichiers et l'IPC, jamais sur le rendu. |
