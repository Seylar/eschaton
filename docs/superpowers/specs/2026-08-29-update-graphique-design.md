...
8. *(ajouté le 2026-08-30, par le défaut du §33.5 de `vm-dev.md`)* **Une annulation ne ment jamais sur ce qu'elle a laissé derrière elle.** Le fichier d'état distingue une annulation arrivée **avant** le point de non-retour de `pacman` d'une arrivée **après**, et cette distinction est **mesurée** — jamais devinée par l'interface. Dans le second cas le panneau le dit et **propose le retour arrière** ; en cas de doute sur ce que pacman a écrit, on retombe sur le pire cas. Corollaire de portée générale : un résultat que le panneau ne connaît pas ne produit **jamais** le message rassurant, et ne perd **jamais** sa porte de sortie.
...
| 14 | *(2026-08-30)* Deux paquets qui portent une même interface se mettent à jour **séparément** : `eschaton-base` écrit le fichier d'état, `eschaton-dms-plugin-update` le lit | Un socle plus récent peut écrire un résultat inconnu du panneau. Deux réponses, et il faut les deux : une **borne de version** dans le PKGBUILD du greffon (`eschaton-base>=0.1.0-23`), et surtout un panneau **fail-closed** — le retour arrière est décidé par une liste d'*exclusion*, si bien qu'un résultat inconnu garde sa porte de sortie au lieu de la perdre en silence. |
...
| 8. **Une annulation ne ment jamais sur ce qu'elle a laissé** | **Satisfait après correction (2026-08-30)** | Ajouté au §5 par le défaut du §33.5, où une annulation tardive annonçait « Rien n'a été installé » alors que `pacman -Q` rendait la version « annulée », **et** ne proposait aucun retour arrière. Le point de non-retour est désormais **mesuré** dans le journal de pacman, aux deux bornes : un `pacman -Sw` intégral — téléchargement, intégrité, signatures — n'écrit rien, et un crochet `PreTransaction` non plus ; `transaction started` puis `upgraded …` n'apparaissent qu'une fois le système modifié. L'état distingue `annule` de `annule-trop-tard`, et le panneau rend les deux différemment. Mesuré aux versions `eschaton-base 0.1.0-23` / `plugin-update 0.1.0-15`, paquets construits par `makepkg` puis installés : cas tardif → `annule-trop-tard`, « Annulation trop tardive : des paquets avaient déjà été installés. », bouton **« Revenir à l'état d'avant »** présent et armé ; cas précoce **non régressé** → `annule`, paquet inchangé, aucun retour arrière proposé. `vm-dev.md` §34 |
...
- ~~**Une annulation tardive ment, et laisse l'utilisateur sans porte de sortie**~~ — **corrigé le 2026-08-30**, voir le point 8 du tableau ci-dessus. Énoncé d'origine, conservé parce qu'il porte la décision de conception : si l'annulation arrivait après le point de non-retour de `pacman` (mesuré : pendant un `post_upgrade`), les paquets **étaient** installés, mais le panneau annonçait « Mise à jour annulée. Rien n'a été installé. » et `restaurationUtile` n'incluait pas `annule` — donc aucun retour arrière n'était proposé, dans le seul cas où l'utilisateur vient d'exprimer qu'il ne voulait pas de cette mise à jour.
