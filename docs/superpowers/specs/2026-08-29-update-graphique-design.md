...
### 3.1 Le piège qui invalide la solution évidente

**`pkexec` ne doit pas porter une opération longue.** Son source arme `prctl(PR_SET_PDEATHSIG, SIGTERM)` — « make sure we are nuked if the parent process dies » — et ce réglage **survit à l'`execv()`** pour un binaire non setuid. Or le « parent » au sens du noyau est **le fil d'exécution**, pas le processus. Notre interface est en QML, donc multi-thread : **la fin d'un simple fil enverrait SIGTERM à `pacman` en pleine transaction**.

C'est négligeable pour le rollback (opération courte, déjà prouvée en réel) — mais catastrophique pour une mise à jour de plusieurs minutes. *Note de vigilance : le rollback conserve ce risque résiduel ; il est accepté au vu de sa durée, et à réexaminer s'il s'allonge.*
...
Le point subtil qui rend ce montage légitime : `systemd-run` **appelé par l'utilisateur** exigerait le droit polkit `org.freedesktop.systemd1.manage-units`, qu'un défaut amont de systemd (#17224) empêche de restreindre à une seule unité — ce serait un passe-partout sur **toutes** les unités système. Appelé **par root après la modale**, il ne déclenche aucun contrôle : l'utilisateur ne reçoit jamais ce droit.
...
- **`systemd-run` côté utilisateur** — écarté par le passe-partout `manage-units` (§3.2).
...
3. La transaction **survit** à la fermeture du panneau et à la fin d'un fil de l'interface — preuve explicite du §3.1.
...
## 6. Risques (table datée 2026-08-29)

| # | Risque | Traitement |
|---|---|---|
| 1 | `systemd-run` lancé par root déclenche malgré tout un contrôle polkit | **Non vérifié — à prouver en tâche 1.** Repli documenté : un petit service D-Bus sur le modèle pamac. |
| 2 | L'utilisateur ne peut pas suivre le journal d'une unité **système** depuis sa session | **Non vérifié — à prouver en tâche 1.** Repli : l'assistant privilégié relaie la sortie. |
| 3 | Détection préalable de l'interactivité de pacman | **Trou de la veille, assumé.** `checkupdates` répond à « y a-t-il des mises à jour ? », pas à « faut-il une décision humaine ? ». Piste : `alpm_trans_prepare()` expose-t-il les conflits avant commit ? Shelly (CachyOS) prouve que la voie libalpm est praticable. |
| 4 | Le risque `PR_SET_PDEATHSIG` subsiste sur le rollback | Accepté (opération courte), consigné §3.1, à réexaminer si sa durée croît. |
