...
Le reste du matériel s'en sort correctement (clavier, trackpad, audio et webcam via `apple-bce` ; Wi-Fi grâce au firmware pré-empaqueté `apple-bcm-firmware`), avec des réserves connues (micro faible, veille et Touch Bar fragiles, GPU AMD dédié à gérer en hybride sur les 15″/16″).
...
1. **Artefacts séparés.** Les adaptations T2 vivent dans un paquet dédié (`eschaton-t2`, forcément non-`any` puisqu'il dépend de `linux-t2`) et un **variant d'ISO** distinct, jamais dans `eschaton-base`, `eschaton-desktop` ni l'ISO principal. La doctrine `arch=(any)` reste la règle ; ce paquet en est **l'exception nommée**, comme les hooks alpm le sont pour « pas de migrations ».
