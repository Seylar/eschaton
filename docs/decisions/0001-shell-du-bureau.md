...
3. **Le critère n°1 de passage au shell maison (§4) est affaibli** *(puis CLOS le 2026-08-28 par la veille SP3 : `DankSlideout` est dans le tag v1.5.3 d'extra, `dms ipc call plugins toggle` existe, et un plugin `daemon` + `Variants{Quickshell.screens}` donne la sidebar plein écran sans mécanique maison — précédent en production. Ce critère ne se déclenchera pas ; les critères 2-4 restent entiers.)* : les cinq types de plugins (`widget`, `daemon`, `launcher`, `desktop`, `composite`) et la permission `process` couvrent a priori l'assistant IA omniprésent. Le critère reste valable, mais sa probabilité de déclenchement baisse nettement.
